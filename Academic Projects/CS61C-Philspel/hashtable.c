...
void insertData(HashTable *table, void *key, void *data) {
  ...
  unsigned int hashLocation = (table->hashFunction)(key);
  unsigned int find = hashLocation % table->size;

  // 2. Allocate a new hash bucket struct.
  struct HashBucket *findBucket = (struct HashBucket *) malloc(sizeof(struct HashBucket));

  // 3. Append to the linked list or create it if it does not yet exist. 
  findBucket->next = table->data[find];
  findBucket->data = data;
  findBucket->key = key;
  table->data[find] = findBucket;
}
...
void *findData(HashTable *table, void *key) {
  ...
  unsigned int hashLocation = (table->hashFunction)(key);
  unsigned int find = hashLocation % table->size;
  struct HashBucket *search = table->data[find];

  // 2. Walk the linked list and check for equality with table->equalFunction.
  while(search != NULL) {
    if ((table->equalFunction)(key, search->key) != 0){
      return search->data;
    }
    search = search->next;
  }
